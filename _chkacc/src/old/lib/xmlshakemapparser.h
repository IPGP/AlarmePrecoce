 ...
 * interest, dealing with any actions required upon detection (SMS file writing)
 *
...
class XMLShakemapParser {
	...
	public:
		...
		struct Station {
				Station(const std::string& c, const std::string& ins,
				        const float& lat, const float& lon, const float& acc) :
