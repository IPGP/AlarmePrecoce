...
bool debug = false;
string configFile, inputEvtFile, inputDatFile, outputFile, stations, polygons;
...
int main(int argc, char** argv) {
	...
	if ( parseOptions(argc, argv) ) {
		...
		if ( !fileExists(inputEvtFile) ) {
			cerr << "Input event's file " << inputEvtFile << " wasn't found, exiting" << endl;
			return 5;
		}

		if ( !fileExists(inputDatFile) ) {
			cerr << "Input file " << inputDatFile << " wasn't found, exiting" << endl;
			return 5;
		}
		...
		XMLShakemapParser parser(set, debug);
		parser.processEvent(inputEvtFile);
		parser.processEventData(inputDatFile);
		...
		switch ( retCode ) {
			...
			case 2:
				retCode = 2;
				cout << "XML file format not recognized, empty " << outputFile << " file written" << endl;
			break;
...
bool parseOptions(int argc, char** argv) {
	...
	try {
		...
		ValueArg<string> inputEvtFileArg("e", "inputEvtFile",
		    "Input XML event file (with path)", true, "inputEvtFile", "string");
		cmd.add(inputEvtFileArg);

		ValueArg<string> inputDatFileArg("i", "inputDatFile",
		    "Input XML file (with path)", true, "inputDatFile", "string");
		cmd.add(inputDatFileArg);

		cmd.parse(argc, argv);

		configFile = configArg.getValue();
		inputEvtFile = inputEvtFileArg.getValue();
		inputDatFile = inputDatFileArg.getValue();

		if ( !configFile.empty() && !inputDatFile.empty() )
		    retCode = true;
	}
